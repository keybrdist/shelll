...
import { 
  ...
  Pin, PinOff, Terminal as TermIcon, Download, 
  Type, Command, Settings, Layout, Ghost
} from "lucide-react";
...
const DEFAULT_FONT = '"JetBrainsMono Nerd Font", "JetBrains Mono", "Apple Color Emoji", monospace';
const FONTS = [
  { name: "JetBrains Mono (Nerd)", value: '"JetBrainsMono Nerd Font", "JetBrains Mono", "Apple Color Emoji", monospace' },
  { name: "Fira Code (Nerd)", value: '"FiraCode Nerd Font", "Fira Code", "Apple Color Emoji", monospace' },
  { name: "Hack (Nerd)", value: '"Hack Nerd Font", "Hack", "Apple Color Emoji", monospace' },
...
const SECRET_PATTERNS = [
  /sk-[a-zA-Z0-9]{20,}T3BlbkFJ/g, // OpenAI (Example)
  /(?:AKIA|ASIA)[0-9A-Z]{16}/g,    // AWS Access Key
  /[a-zA-Z0-9-_]{20,}\.[a-zA-Z0-9-_]{6,}\.[a-zA-Z0-9-_]{20,}/g, // JWT-like
];
...
export default function App() {
  ...
  // Update font when state changes
  useEffect(() => {
    if (xtermRef.current) {
        xtermRef.current.options.fontFamily = fontFamily;
        // Trigger resize to ensure everything reflows/renders correctly
  ...
  const scanBlocks = useCallback(() => {
    ...
    function addBlock(startRel: number, endRel: number, lines: string[]) {
        ...
        const clientHeight = element.querySelector('.xterm-screen')?.clientHeight || element.clientHeight;
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    // Offset for header
    const y = e.clientY - 40; // Approx header height + top padding
  ...
  return (
    <div 
        className={clsx(
            ...
            // Increased opacity to 70% to fix "too bright" issue while keeping vibrancy
            "bg-neutral-950/70", 
    ...
    >
        {/* Header */}
        <div data-tauri-drag-region className="h-10 flex items-center justify-between px-4 border-b border-white/5 bg-white/5 select-none z-50">
            <div className="flex items-center gap-4">
                ...
                <button 
                    ...
                    className={clsx("transition-colors", isRedactMode ? "text-green-400" : "text-red-400 opacity-50 hover:opacity-100")}
                ...
                <button 
                    onClick={togglePin}
                    className={clsx("transition-colors", isPinned ? "text-blue-400" : "text-white/40 hover:text-white")}
                ...
                <button 
                    ...
                    className={clsx("transition-colors", isGhostMode ? "text-purple-400" : "text-white/40 hover:text-white")}
            ...
            <div className="flex items-center gap-2">
                ...
                <button 
                    ...
                    className={clsx("p-1.5 rounded hover:bg-white/10 transition-colors", showFontSettings ? "text-white bg-white/10" : "text-white/40 hover:text-white")}
                    title="Font Settings"
                >
                    <Settings size={14} />
                </button>
                ...
                <button 
                    ...
                    className={clsx("p-1.5 rounded hover:bg-white/10 transition-colors", showResizeSettings ? "text-white bg-white/10" : "text-white/40 hover:text-white")}
                    title="Window Size"
                >
                    <Layout size={14} />
                </button>
                ...
                <button 
                    ...
                    className={clsx("p-1.5 rounded hover:bg-white/10 transition-colors", showCmdPalette ? "text-white bg-white/10" : "text-white/40 hover:text-white")}
                ...
                >
                    {copiedId === 'all' ? <Check size={14} className="text-green-400" /> : <Clipboard size={14} />}
                    <span>Copy All</span>
                </button>
            </div>
        </div>
      ...
      {/* Terminal Container */}
      <div className="flex-1 relative p-4 pl-6 overflow-hidden"> 
        ...
        <div className="absolute top-4 left-6 right-4 bottom-4 pointer-events-none">
            {blocks.map(block => {
                ...
                // Only show interactions if hovered or selected
                    ...
                    >
                        ...
                        <div 
                            className={clsx(
                                ...
                                isSelected ? "bg-blue-500/10 border-blue-500/20" : "bg-white/[0.03] border-white/5"
                            )}
                        />
                  ...
                  {FONTS.map(font => (
                      <button
                        ...
                        className={clsx(
                            ...
                            fontFamily === font.value ? "bg-white/10 text-white" : "text-white/60 hover:bg-white/5 hover:text-white"
                        )}
                        style={{ fontFamily: font.value }}
                      >
                          {font.name}
                          {fontFamily === font.value && <Check size={12} className="text-green-400" />}
                      </button>
                  ))}
              </div>
      ...
      {showResizeSettings && (
          <div className="absolute top-12 right-20 w-48 bg-neutral-900/95 backdrop-blur-xl border border-white/10 rounded-lg shadow-2xl p-1 z-50 animate-in fade-in zoom-in-95 duration-100">
              <div className="text-xs font-medium text-white/40 px-2 py-1 mb-1">WINDOW PRESETS</div>
              {WINDOW_PRESETS.map(preset => (
                  ...
                  >
                      ...
                      <span className="text-white/20 text-xs font-mono">{preset.width}x{preset.height}</span>
                  </button>
              ))}
          </div>
      )}
      ...
      {showCmdPalette && (
          <div className="absolute top-12 left-1/2 -translate-x-1/2 w-64 bg-neutral-900/95 backdrop-blur-xl border border-white/10 rounded-lg shadow-2xl p-1 z-50 animate-in fade-in zoom-in-95 duration-100">
              <div className="text-xs font-medium text-white/40 px-2 py-1 mb-1">QUICK COMMANDS</div>
              {[
